...
- **Auth**: Replit OIDC with PKCE via `openid-client` and `iron-session`. Login/callback routes derive the callback origin from the active request host so preview, mobile, and deployed domains keep the session cookie on the same host. Protected app pages and normal `/sign-in` visits redirect directly to `/api/login`; `/sign-in` only renders a fallback page for auth-start/callback errors. Session cookie: `replit_auth_session`
