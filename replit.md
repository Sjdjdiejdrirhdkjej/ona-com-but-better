...
- **Auth**: Replit OIDC with PKCE via `openid-client` and `iron-session`. Login/callback routes derive the callback origin from the active request host so preview, mobile, and deployed domains keep the session cookie on the same host. All sign-in entry points first land on `/sign-in?returnTo=...`, which auto-starts `/api/login` from a stable app-owned page and keeps a visible retry button. The callback saves the session, returns a small "continue to ONA" handoff page, and attempts top-level/opener navigation back to the intended app path with a manual fallback link for mobile auth tabs. Session cookie: `replit_auth_session`
