...
- **Auth**: Replit OIDC with authorization code + PKCE + nonce via `openid-client` and `iron-session`, using issuer `https://replit.com/oidc` and dynamic client registration. Login/callback routes derive the callback origin from the active request host so preview, mobile, and deployed domains keep the session cookie on the same host; Replit preview/app hosts are accepted explicitly for browser compatibility. Mobile Get Started, mobile menu, protected-route redirects, and mobile client sign-in calls route through `/sign-in?returnTo=...`, where users explicitly open Replit sign-in in a separate tab/window while the ONA tab stays alive and polls `/api/auth/user?optional=1` with bounded retries. Desktop Get Started/client sign-in preserves the prior direct `/api/login?returnTo=...` start from the user's click, and desktop arrivals at `/sign-in` without an error automatically continue to `/api/login`. Sign-in error pages broadcast provider/callback failures back to the waiting tab via BroadcastChannel/localStorage so users see retry/continue actions before timeout. The callback saves the session, returns a same-origin "continue to ONA" handoff page with a manual Continue button, uses popup postMessage/close only when a safe desktop opener exists, never attempts to close mobile auth tabs, and gives desktop non-popup callbacks a short visible continuation page before redirecting. Session cookie: `replit_auth_session` with SameSite=Lax.
